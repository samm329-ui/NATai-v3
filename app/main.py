...
async def lifespan(app: FastAPI):
    ...
    logger.info(
        "[CONFIG] Tavily API key: %s", "Configured" if TAVILY_API_KEY else "NOT SET"
    )
    logger.info(
        "[CONFIG] Vision Model: %s", "Configured" if GROQ_API_KEYS else "NOT SET"
    )
...
async def chat(request: ChatRequest):
    ...
    logger.info(
        "[API] /chat incoming | session_id: %s, message_len=%d",
        request.session_id,
        len(request.message),
    )
    ...
    try:
        ...
        logger.info(
            "[API] /chat response | session_id: %s, response_len=%d",
            session_id,
            len(response_text),
        )
...
def _merge_short(sentences):
    ...
    while i < len(sentences):
        ...
        while (
            i + 1 < len(sentences) and len(sentences[i + 1].split()) <= MERGE_IF_WORDS
        ):
            i += 1
            cur += " " + sentences[i]
...
def _stream_generator(
    session_id: str, chunk_iter, is_realtime: bool, tts_enabled: bool = False
):
    ...
    def _submit(text):
        ...
        audio_queue.append(
            tts_pool.submit(generate_tts_sync, text, TTS_VOICE, TTS_RATE)
        )
        last_submit_time = time.perf_counter()
    ...
    def _drain_ready():
        while audio_queue and audio_queue[0].done():
            ...
            try:
                audio = fut.result(timeout=0.1)
                if audio:
                    b64 = base64.b64encode(audio).decode("ascii")
                    events.append(
                        f"data: {json.dumps({'audio': b64, 'sentence': ''})}\n\n"
                    )
            except FuturesTimeoutError:
                audio_queue.insert(0, fut)
                break
            except Exception as e:
                logger.warning("[TTS-INLINE] Failed for audio: %s", e)
    ...
    if tts_enabled:
        ...
        for fut in audio_queue:
            try:
                ...
                logger.warning("[TTS-INLINE] Timeout for audio generation")
            except Exception as e:
                logger.warning("[TTS-INLINE] Failed for audio: %s", e)
...
async def chat_stream(request: ChatRequest):
    ...
    logger.info(
        "[API] /chat/stream incoming | session_id: %s, message_len=%d",
        request.session_id,
        len(request.message),
    )
...
async def chat_realtime(request: ChatRequest):
    ...
    logger.info(
        "[API] /realtime incoming | session_id: %s, message_len=%d",
        request.session_id,
        len(request.message),
    )
...
async def chat_realtime_stream(request: ChatRequest):
    ...
    logger.info(
        "[API] /realtime/stream incoming | session_id: %s, message_len=%d",
        request.session_id,
        len(request.message),
    )
    ...
    try:
        ...
        return StreamingResponse(
            _stream_generator(
                session_id, chunk_iter, is_realtime=True, tts_enabled=request.tts
            ),
            media_type="text/event-stream",
            headers={"Cache-Control": "no-cache", "X-Accel-Buffering": "no"},
        )
...
async def natasha_stream(request: ChatRequest):
    ...
    logger.info(
        "[API] /natasha/stream incoming | session_id: %s, message_len=%d, img=%s",
        request.session_id,
        len(request.message),
        "yes" if request.img_base64 else "no",
    )
    ...
    async def generate():
        try:
            ...
            communicate = edge_tts.Communicate(
                text=request.text, voice=TTS_VOICE, rate=TTS_RATE
            )
