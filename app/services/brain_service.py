...
_PRIMARY_BRAIN_PROMPT = """You are the decision-maker for NATASHA. Classify the user's message into EXACTLY ONE category.
...
- Example: User said "open natasha4everyone.com", assistant opened wrong site, user says "it's integer 4 not f-o-r" → STILL a task
...
_TASK_BRAIN_PROMPT = """You are a very accurate Decision-Making Model for NATASHA. You decide what kind of task the user wants and extract the CLEAN query/topic for each task.
...
-> 'open (website/app name)' — Open a website or app.
   ...
   "Open natasha4everyone.com" → open natasha4everyone.com
...
-> 'play (song/video name)' — Play music or video on YouTube.
   "Play Dhurandhar title track" → play Dhurandhar title track
    "Hello Natasha can you play Shape of You on YouTube" → play Shape of You
...
-> 'google_search (search topic)' — Search something on Google.
   ...
   "Google what is quantum computing" → google_search what is quantum computing
    "Hey Natasha look up best restaurants nearby" → google_search best restaurants nearby
...
- "it's integer 4 not f-o-r" (after "open natashaforeveryone.com") → open natasha4everyone.com
...
=== CRITICAL RULES ===
*** Extract ONLY the relevant topic/query — REMOVE greetings (hello, hey, hi), assistant name (Natasha), filler words (can you, please, for me), platform names (on YouTube, on Google), and command words from the query. ***
...
Classify. Output EXACTLY ONE category name."""
    ...
    SITE_MAP = {
        ...
        "natasha4everyone": "https://natasha4everyone.com",
        "natasha4everyone.com": "https://natasha4everyone.com",
        "my website": "https://natashaforeveryone.com",
        "natashaforeveryone.com": "https://natashaforeveryone.com",
    }
