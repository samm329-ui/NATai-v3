...
class VectorStoreService:
        ...
        for file_path in sorted(LEARNING_DATA_DIR.glob("*.txt")):
            try:
                 ...
                 if content:
                     ...
                     logger.info("[VECTOR] Loaded learning data file: %s", file_path.name)
    ...
    def _create_vector_store(self) -> FAISS:
        ...
        logger.info("[VECTOR] Split %d documents into %d chunks (chunk_size=%d, overlap=%d)", len(all_documents), len(chunks), CHUNK_SIZE, CHUNK_OVERLAP)
    ...
    def get_retriever(self, k: int = 10):
        if not self.vector_store:
            try:
                 self.vector_store = FAISS.load_local(str(VECTOR_STORE_DIR), self.embeddings, allow_dangerous_deserialization=True)
                 logger.info("[VECTOR] Loaded existing FAISS index from disk.")
            except Exception as e:
                 logger.info("[VECTOR] Could not load FAISS from disk (%s). Creating new index...", e)
        ...
        if k not in self.retriever_cache:
            self.retriever_cache[k] = self.vector_store.as_retriever(search_kwargs={"k": k})
