...
class GroqService:
    def __init__(self, vector_store_service: VectorStoreService):
        ...
        self.prompt_template = PromptTemplate.from_template(NATASHA_SYSTEM_PROMPT)
        self.chain = None

    def _init_llms(self):
        if not GROQ_API_KEYS:
            logger.error(
                "No GROQ API keys configured. Set GROQ_API_KEY (and optionally GROQ_API_KEY_1, ...) in .env"
            )
            return
        ...
        for i in range(n):
            ...
            try:
                ...
                if res and detect_repetition_loop(res):
                    logger.warning(
                        "[GROQ] Repetition loop detected - truncating response (50 chars): %s",
                        res[:50],
                    )
                ...
                if i > 0:
                    logger.info(
                        "Fallback successful! API key #%d (%s) succeeded.",
                        idx + 1,
                        masked_key,
                    )
                ...
                if is_rate_limit_error(e):
                    logger.warning(
                        "API key #%d (%s) rate limited.", idx + 1, masked_key
                    )
                else:
                    logger.warning(
                        "API key #%d (%s) failed: %s", idx + 1, masked_key, str(e)[:100]
                    )
    ...
    def _stream_llm(
        ...
        for i in range(n):
            ...
            try:
                ...
                for chunk in chain.stream({"history": messages, "question": question}):
                    ...
                    if len(accumulated) - last_check_len > REPEAT_CHECK_INTERVAL:
                        ...
                        if detect_repetition_loop(accumulated):
                            logger.warning(
                                "[GROQ] Repetition loop detected after 50 chars - stopping: %s",
                                accumulated[:50],
                            )
                ...
                logger.info(
                    "Stream total: %.2fs, chunks: %d%s",
                    total_stream_time,
                    chunk_count,
                    ", TRUNCATED: REPETITION" if repetition_stopped else "",
                )
    ...
    def _build_prompt_and_messages(
        ...
        try:
            ...
            logger.warning(
                "[VECTOR] Vector retrieval failed, using empty context: %s", e
            )
        ...
        logger.info(
            "[PROMPT] System message length: %d chars | History pairs: %d | Question: %.50s...",
            len(system_message),
    ...
    def get_response(
        ...
        try:
            ...
            logger.info(
                "[RESPONSE] General chat: %d chars | Preview %.50s...",
                len(result),
                result,
            )
            return result
