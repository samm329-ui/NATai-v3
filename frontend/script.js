...
let ttsPlayer = null;
const SETTINGS_KEY = 'natasha_settings';
...
function init() {
    if (!chatMessages || !messageInput) {
        console.error('[NATASHA] Required DOM elements (chat-messages, message-input) not found.');
        return;
    }
    loadSettings();
...
function addMessage(role, text) {
    ...
    label.textContent = role === 'assistant'
        ? `Natasha  (${currentMode === 'natasha' ? 'Natasha' : currentMode === 'realtime' ? 'Realtime' : 'General'})`
        : 'You';
...
function addTypingIndicator() {
    ...
    const label = document.createElement('div');
    label.className = 'msg-label';
    label.textContent = `Natasha  (${currentMode === 'natasha' ? 'Natasha' : currentMode === 'realtime' ? 'Realtime' : 'General'})`;
    const content = document.createElement('div');
    content.className = 'msg-content';
